...
//前置登录信息传入mainwindow
void MainWindow::sendUserData(const QList<QStringList>  data)
{
    //当前用户属性，全局变量
    qDebug()<<"登录向主界面传值";
    myName = data[0][1];
    usrInfo = data; //将所有信息赋值给全局变量
    qDebug()<< usrInfo;
    //绑定个人资料控件
    qDebug()<<"登录向主界面传值1";
    QPixmap pix;
    ...
    ui->birthdayLabel->setText("生日: "+data[0][4].mid(0, 10));
    ui->EmailLabel->setText("邮箱: "+data[0][6]);
    ui->PhoneLabel->setText("联系方式: "+data[0][5]);
...
//好友列表加载
void MainWindow:: getFriendsList(){

    //遍历usrsInfo，显示所有好友的信息
    for(int i= 1;i<usrInfo.size();i++)
    {
         headpics.append(usrInfo[i][7]);
         names.append(usrInfo[i][1]);
         userSignal.append(usrInfo[i][2]);
    }
}
...
void MainWindow::sendChatMsg(ChatMsgType msgType, QString rmtName,QStringList QbuffList)
{
    ...
    {
    case ChatMsg:
        write << myName<<rmtName<<locHostIp << locChatMsg;  //2、在发送的信息中，写入 本机ip+聊天信息
        break;
    case SfileName:
        //2、写入发送端的Ip、文件名
        write <<myName<<rmtName<< locHostIp << myFileName;
        break;
    case RefFile:
        write <<myName<<rmtName<< locHostIp << rmtName;
        break;
    case FriendAdd: //好友申请
        write <<myName<<rmtName<<QbuffList<<locHostIp; //2、IP
        break;
    case RefFriend: //拒绝好友申请
        write <<myName<<rmtName<<locHostIp;
        break;
    case AcpFriend: //本地UDP同意好友申请
        write <<myName<<rmtName<<QbuffList<<locHostIp;//附加好友个人信息
        break;
    case acpFriend:
        //被申请者向服务器发送的信号
        write<<rmtName<<myName;//申请者+被申请者
    }       
    myUdpSocket->writeDatagram(qba, qba.length(), QHostAddress::Broadcast, 5000);  //完成消息处理后，发出广播
}
...
void MainWindow::recvAndProcessChatMsg()
{
    while (myUdpSocket->hasPendingDatagrams()) //判断是否读到消息
    {
        ...
        QStringList revPeerInfo,friInfoList;
        ...
        case FriendAdd:
        {
           ...
           {recvFriendAdd(udpmyname,udpclickname,revPeerInfo);} //申请者+申请要加的人+申请者的信息表
        }
           break;
        case SfileName:
...
//            }
            if(myName==udpmyname|myName==udpclickname)
            {
                //向服务器发送，更新好友列表
                sendChatMsg(getFriends,myName);
            }

            break;
        case getFriends:
            read>>udpmyname>>udpclickname>>info;
            ...
            break;
        case updateDataBase:
            read>>udpmyname>>udpclickname;
            if(myName==udpmyname||myName==udpclickname)
            {
                sendChatMsg(getFriends,myName);//向服务器请求更新列表
            }
            break;
    ...
    if(udpclickname==myName)
    {
        ...
        if (result == QMessageBox::Yes)
        {
            //发送同意加好友 +并附上个人信息，QStringList==usrinfo
             ...
             //被申请方，向服务器发送同意加好友
             sendChatMsg(acpFriend,udpmyname);
             //将申请者加入数据库
